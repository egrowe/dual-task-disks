...
nTrials = 64;   % Number of trials per block (min = 8)
...
crit = nTrials/4;
% Controlled randomization of central task stimuli
% splitTrials = nTrials/4; %split trials to Ls, Ts, L T or T L
% periphOrder = [ones(1,splitTrials), ones(1,splitTrials)*2, ones(1,splitTrials)*3, ones(1,splitTrials)*4]
% periphOrder = Shuffle(periphOrder);

% Controlled randomization of peripheral task
periphTypeLeft = Shuffle([ones(1,nTrials/2)+1, zeros(1,nTrials/2)+1])
periphTypeRight = Shuffle(periphTypeLeft);

%Re-define these as the same trial types as in the main exp
for ii = 1:length(periphTypeLeft)
    if periphTypeLeft(ii) == 1
        periphPairsLeft(ii) = 1;
    elseif periphTypeLeft(ii) == 2
        periphPairsLeft(ii) = 5;
    end
end

for ii = 1:length(periphTypeRight)
    if periphTypeRight(ii) == 1
        periphPairsRight(ii) = 1;
    elseif periphTypeRight(ii) == 2
        periphPairsRight(ii) = 5;
    end
end
...
for tr = 1 : nTrials
   ...
   TR(tr).periphOrder = [periphPairsLeft(tr), periphPairsRight(tr)]
...
%     
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    TR(tr).periphTypeLeft = periphTypeLeft(tr)
    TR(tr).periphTypeRight = periphTypeRight(tr)
    %%%%%%%%%%%%%%%%%%%% LEFT %%%%%%%%%%%%%%%%%%%%%%%
    ...
    %Set mondrian image to show on each side
